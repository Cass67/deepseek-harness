...
Both wire ends share one method set: the three client-to-server prompt-loop requests and four server-to-client notifications below, plus the fork L2 application-control requests (`settings/get`, `settings/set`, `skills/list`, `agent-presets/list`, `session/list`, `session/history`, `session/resume`, `session/cancel`, `session/close`, `session/selectModel`, `command/list`, `command/execute`, `interaction/respond`, `llm/catalog`, `attachment/imageLimits`, `attachment/saveImage`, and the five `provider/auth*` methods) and two L2 notifications (`interaction.requested` addressing approval/user-question waits, and the `provider.auth.*` event stream for login flows).
...
- **Cancellation is session-activity scoped, not prompt scoped** — `session/cancel` requests whole-agent activity cancellation and returns a boolean acknowledgement; clients observe whole-agent status and durable events.
- **Interactions are notification-correlated** — approval and user-question waits surface as `interaction.requested` notifications, and `interaction/respond` is client→server; they are not ordinary server→client JSON-RPC requests.
