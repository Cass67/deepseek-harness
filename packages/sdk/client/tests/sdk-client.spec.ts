...
describe('HarnessClient', () => {
  it('projects every interactive control method with validated typed results', async () => {
    const client = createProcessHarnessClient(fakeLaunch())
  ...
  it('validates optional session headers and exact contiguous history', async () => {
    const client = createProcessHarnessClient(fakeLaunch({ FAKE_OPTIONAL_SESSION_HEADER: '1' }))
    cleanups.push(() => client.close())
  ...
  it.each([
    ...
    const client = createProcessHarnessClient(fakeLaunch(environment))
    cleanups.push(() => client.close())
  ...
  it('rejects malformed or request-mismatched attachment responses', async () => {
    const client = createProcessHarnessClient(fakeLaunch({ FAKE_MALFORMED_ATTACHMENTS: '1' }))
    cleanups.push(() => client.close())
    ...
    const mismatch = createProcessHarnessClient(fakeLaunch({ FAKE_MISMATCH_ATTACHMENT: '1' }))
  ...
  it('rejects malformed catalog reasoning metadata', async () => {
    const client = createProcessHarnessClient(fakeLaunch({ FAKE_MALFORMED_REASONING: '1' }))
    cleanups.push(() => client.close())
  ...
  it('rejects malformed interactive control responses', async () => {
    ...
    for (const call of calls) {
      const client = createProcessHarnessClient(fakeLaunch({ FAKE_MALFORMED_CONTROLS: '1' }))
      cleanups.push(() => client.close())
  ...
  it('rejects malformed provider auth results and notifications', async () => {
    const malformedInfo = createProcessHarnessClient(fakeLaunch({ FAKE_MALFORMED_AUTH: '1' }))
    ...
    const malformedNotification = createProcessHarnessClient(fakeLaunch({ FAKE_MALFORMED_AUTH: '1' }))
    ...
    for (const call of malformedResults) {
      const client = createProcessHarnessClient(fakeLaunch({ FAKE_MALFORMED_AUTH_RESULTS: '1' }))
      cleanups.push(() => client.close())
      await expect(call(client)).rejects.toBeInstanceOf(SdkProtocolError)
      await client.close()
    }
