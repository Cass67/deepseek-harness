...
The plugin creates one agent per `sessionId` on first use. A registered model adapter wins the route; an unowned `deepseek-official` route mounts the DeepSeek adapter, and any other unowned provider fails initialization. The selected adapter resolves the exact model and optional reasoning effort before initialization succeeds. Attachment, LLM, command, session-query, approval, and user-question services are read as optional capabilities: each L2 method section below fails loud when its required service is absent from the composition.
...
`initialize` is the runtime-readiness boundary: when the server is mounted by a Loader composition, it waits for the current plugin tree to settle before replying, so async sibling capabilities such as initial MCP tool discovery are visible to the first prompt. The handshake returns the wire-stable identity `deepseek-harness-sdk-runtime`. The server validates the provider/model route and optional non-empty `reasoningEffort` through the selected adapter before it stores them; omission stores no effort, so the model retains its own default. An optional positive `maxTokens` becomes the request output cap of each SDK-created agent and its in-process descendants, while omission applies the selected adapter or provider route default. JSON-RPC requests may dispatch concurrently, so `session/prompt` must be preceded by a successful `initialize`; it queues one identified user message and immediately returns `{ messageId }`, then the server streams every durable fact as `session.event` and every whole-agent lifecycle transition as `session.status`. It does not assign an assistant message or `turn/end` to a prompt, and independent requests may enqueue more work on the same session. Persistence roots and persona come from the surrounding composition.

Beyond the prompt loop, the server dispatches fork L2 application-control methods. `provider/authInfo`, `provider/authStart`, `provider/authRespond`, `provider/authCancel`, and `provider/authLogout` expose non-secret authentication status and drive provider-native login flows (API-key masked entry and OAuth/device/manual-code) owned by the mounted adapter; submitted values are never copied into notifications or diagnostics, and terminal failures use a generic message. `attachment/imageLimits` returns the active upload policy, and `attachment/saveImage` publishes canonical base64 to the durable store and returns an opaque reference; both fail loud without a mounted attachment service. `llm/catalog` lists every registered provider with per-model modality, metadata, and adapter-owned reasoning efforts. `session/list` and `session/history` read the session-query service without acquiring an agent, `session/resume` is an explicit `ctx.agents.resume` operation that restores the latest logged route, `session/selectModel` validates through the LLM service and changes the mutable per-step selection, `session/cancel` requests whole-agent cancellation without touching queued work, and `session/close` aborts active command dispatches and awaits server-owned session disposal before permitting clean recreation. `command/list` and `command/execute` discover and run scope-effective slash commands through the command registry with durable lifecycle, never as a model-visible message. `settings/get` and `settings/set` read namespaces and apply `patch`/`replace` with an optional revision guard; `skills/list` and `agent-presets/list` enumerate their registries. Approval and user-question requests become `interaction.requested` notifications, and `interaction/respond` validates correlation and exact question choices before the first valid response claims the wait; cancellation, session closure, and server disposal settle pending interactions.
...
- **Cancellation is session-activity scoped, not prompt scoped** — the SDK `session/cancel` requests whole-agent activity cancellation and returns a boolean acknowledgement; clients observe whole-agent status and durable events.
