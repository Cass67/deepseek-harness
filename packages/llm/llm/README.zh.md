...
`LlmAdapter.authInfo/login/logout` 与配套的 `LlmRuntime` 转发构成了提供方中立的原生认证接口。元数据只包含方法标签、配置类型/来源与状态。登录交互携带文本/密钥/选择/手动码提示，以及信息、授权 URL、设备码与进度事件；适配器负责协议机制，调用方负责呈现与关联。默认适配器方法报告不可用，因此没有原生认证的适配器不会获得伪造的控制能力。
