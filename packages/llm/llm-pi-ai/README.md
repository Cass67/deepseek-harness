...
Configure credentials, the model catalog, and deployment-specific transport settings per provider, keyed by the provider route itself. `apiKeyEnv` is a credential *reference* resolved per request, so no secret enters this file. Omitting it uses pi-ai's durable provider-native credential and then its ambient discovery; a configured reference that resolves to nothing fails the request with `MISSING_CREDENTIAL` instead, because falling through would authenticate with whatever unrelated key the environment happens to hold. One credential serves every model on its route.
...
The adapter exposes provider-neutral auth metadata and operations through `ctx.llm`: API-key and OAuth method labels, non-secret configured/source status, login prompts/events, and logout. Login orchestration remains surface-owned while pi-ai owns provider protocols, callback servers, device polling, token exchange, and refresh. Every immutable model snapshot receives the same durable credential store, so a stored OAuth token refreshes under one cross-process provider lock and the next request observes the committed credential.

The store path is `$DSH_PI_AI_AUTH_PATH`, else `$DSH_HOME/pi-ai-auth.json`, else `~/.dsh/pi-ai-auth.json`. Its directory/file are created or required as `0700`/`0600`; writes use a sibling lock plus atomic rename. JSON is strictly validated before any value reaches pi-ai. Credential values never appear in status metadata, logs, or auth notifications. A profile with `apiKeyEnv` keeps fail-loud named-reference behavior and does not silently fall through to stored/native auth; provider-native interactive auth applies to profiles that omit `apiKeyEnv`.
...
Every collection shares one durable provider-keyed credential store. Provider-native API-key and OAuth login, refresh, request authentication, and logout therefore survive configuration snapshots and process restarts. A profile that explicitly names `apiKeyEnv` still resolves that harness credential reference per request and passes it as pi-ai's highest-priority `apiKey` override; a missing named reference fails loud rather than falling through to a stored or ambient credential. A profile naming no reference uses the native store first and then the provider's ambient discovery.
...
- **Native credentials are protected by filesystem permissions, not OS keychain encryption** — API keys and OAuth refresh/access tokens live in the JSON file selected by `DSH_PI_AI_AUTH_PATH` (then `$DSH_HOME/pi-ai-auth.json`, then `~/.dsh/pi-ai-auth.json`). The directory and file are created or required as `0700`/`0600`, and writes are atomic and cross-process serialized, but a process running as the same user can read them.
...
- **A keyless local route still needs an authentication posture** — a hand-declared route with no `apiKeyEnv` offers native masked API-key login and remains unconfigured until login completes. A truly keyless server instead needs an `Authorization` entry in `headers` (a placeholder value is sufficient when ignored).
