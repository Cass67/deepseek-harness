...
By default, the SDK launches the bundled single-file `dsh-jsonrpc-agent` executable from the `deepseek-harness-runtime-bin` package and injects that package's default configuration (the stdio JSON-RPC server, agent core, preloaded DeepSeek adapter, owner-private local attachment storage under `DSH_HOME`, JSONL session persistence with an explicitly composed semantic checkpoint policy, local bash) via `DSH_CORDIS_CONFIG`. To run a plugin composition of your own, keep the `@deepseek-ai/dsh-sdk-jsonrpc-server` entry in the config and pass the Cordis config path.
...
`DeepSeekHarness.image_limits()` returns the active image policy, and `save_image(data, media_type, name=None)` uploads encoded bytes and returns a strict `ImageAttachmentRef` for a later `{"type": "image", "attachment": ref.model_dump(by_alias=True, exclude_none=True)}` content block. The low-level client exposes the same methods; both reject malformed references and references whose byte count or media type differs from the request. Image content also requires the selected runtime LLM adapter and model route to accept image input; durable attachment storage alone does not add image support.

Low-level provider-native authentication mirrors the TypeScript SDK: `provider_auth_info()`, `start_provider_auth()`, `respond_provider_auth()`, `cancel_provider_auth()`, and `logout_provider()`. Pydantic validates all non-secret result shapes; submitted keys/codes are request-only values and never appear in returned models.

`HarnessClient` retains discovered subagent ancestry for the lifetime of the runtime process. During each `Session.run()`, `RunResult.notifications` and `on_notification` receive the root session and all known descendant notifications in wire order, including nested subagent lifecycle and session events. `RunResult.events` contains root-session events only, so descendant messages cannot replace the root response. The low-level `session_prompt()` returns the queued `MessageId` immediately; callers that bypass `Session.run()` own any later activity boundary themselves. Interactive clients use `list_models()` for the provider/model catalog, `list_sessions()` and `session_history()` for non-attaching durable reads, explicit `resume_session()` to rebuild a persisted agent, `select_model(session_id, provider, model, reasoning_effort=None)` for later steps, `cancel_session()` for cancellation acknowledgement without idle completion, and `close_session()` for quiescent agent teardown. `interaction.requested` notifications are answered with `respond_approval()`, `respond_question()`, or `cancel_question()`; stable request ids and exact option validation make the first valid response win. `list_commands()` and `execute_command()` dispatch through the runtime command registry, so `/compact` and plugin commands do not become model-visible user messages. These methods return exported Pydantic response models and reject malformed runtime responses; catalog failures remain per provider, and command capability absence is a structured execution outcome.
